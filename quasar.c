...
int main(int argc, char **argv){
  ...
  uint64_t	allrps, allbps, alltwoh;
  float		alllat;
  ...
  if (argc < 4) {
  meh:		fputs("Usage: ./quasar <#threads> <#runtime> <URL> <fill | read> <#reportfreq>\n", stderr);
    exit(1);
  }
  if ( (sscanf(argv[1], "%d", &tnum) < 1) || (sscanf(argv[2], "%d", &runtime) < 1) ) goto meh;
  if (tnum < 1) {
  ...
  fprintf(stdout, "------------------------------------------------------------\n");
  fprintf(stdout, " Elapsed (s)       RPS      200OKs       kbps      Lat (ms)\n");
  ...
  for (; running;) {
    ...
    alllat = (allrps) ? (float)tick.tv_sec / (float)alltwoh / (float)updfreq : 0;
